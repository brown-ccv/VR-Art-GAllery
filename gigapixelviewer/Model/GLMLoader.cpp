...
Model* GLMLoader::loadModel(std::string& filename)
{
	...
	/* open the file */
	std::string resourcesModelFolderPath = "Resources/Models/";
	std::string textureFullPath = resourcesModelFolderPath.append(filename);

	file = fopen(resourcesModelFolderPath.c_str(), "r");
	if (!file)
	{
		std::cerr << "unable to load " + resourcesModelFolderPath << std::endl;
		assert(false );
	}
